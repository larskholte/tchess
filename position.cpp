...
Move Position::moveToPositionForColor(const Position &pos, color c) const {
    ...
    if (CountBitsU64(ourVacancies) == 2) {
        // Castle
        Move m;
        if ((ourChanges & 0x0f) && c == white) m.special = smCastleKingside;
        else if ((ourChanges & 0xf0) && c == white) m.special = smCastleQueenside;
        else if ((ourChanges & 0x0f00000000000000) && c == black) m.special = smCastleKingside;
        else if ((ourChanges & 0xf000000000000000) && c == black) m.special = smCastleQueenside;
